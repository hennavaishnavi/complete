...
#line (1, 5) - (1, 23) 30 ""Original.razor""
        ...
        [TestMethod]
        public void IsWithinGeneratedContent_GeneratedWithMappedLocation_ShouldReturnFalse()
        {
            var code = @"// Some code";
            var location = Location.Create(CSharpSyntaxTree.ParseText(code).WithFilePath("Program.razor"), new TextSpan(0, 12));

            var result = location.IsWithinGeneratedContent();

            result.Should().BeFalse();
        }

        [TestMethod]
        public void IsWithinGeneratedContent_GeneratedLocation_ShouldReturnTrue()
        {
            var code = @"// Some code";
            var location = Location.Create(CSharpSyntaxTree.ParseText(code).WithFilePath("Program.razor.g.cs"), new TextSpan(0, 12));
        ...
        [TestMethod]
        public void IsWithinGeneratedContent_LocationNull_ShouldReturnFalse()
        {
            Location location = null;
